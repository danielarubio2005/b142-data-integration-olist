...
# Create the analytics volume in Catalog UI first if you haven't
...
# Read back what we just wrote and confirm it's good
...
print(f"Date range: {fact.agg({'order_purchase_timestamp':'min'}).collect()[0][0]} to {fact.agg({'order_purchase_timestamp':'max'}).collect()[0][0]}")